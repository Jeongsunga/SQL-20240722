...
-- 2. 이미 인증 절차가 진행 중인 전화번호가 있다면 해당 전화번호에 대한 인증 번호를 수정하는 작업으로 대체
...
-- 고객 테이블에서 전체 레코드의 개수와 전체 레코드의 개수에 10을 나눈 값에 소수점 첫 번째 자리에서 올림한 값을 조회
...
-- 필요한 추가 데이터 : 페이지 번호 (화면에서 가져옴), 페이지 번호에 해당하는 시작 위치 (서버가 생성)
SELECT C.customer_number, C.name, C.birth, C.area, N.name
FROM customer C INNER JOIN nurse N
ON C.charger = N.id
...
-- 고객 상세보기
-- 최종 사용자 (고객 정보) => (고객번호, 고객 사진, 고객 이름, 생년월일, 담당자 이름, 주소)
