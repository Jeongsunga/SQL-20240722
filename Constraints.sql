...
-- NOT NULL 제약조건이 걸린 컬럼에 값을 지정하지 않음(doesn't hava a default value)
INSERT INTO not_null_table (null_column) VALUES (1);
-- NOT NULL 제약조건이 걸린 컬럼에 null을 지정함(not_null_column cannot be null)
...
-- UNIQUE로 지정된 칼럼은 중복된 데이터로 수정할 수 없음
...
INSERT INTO key_table VALUES(null, 1);	-- null 불가
INSERT INTO key_table (surrogate_column) VALUES (1);  -- null 불가

INSERT INTO key_table VALUES (1, 1);
INSERT INTO key_table VALUES (1, 2);  -- 중복 불가
...
-- FOREIGN KEY 제약조건을 특정 컬럼에 적용할 때는 반드시 데이터 타입이 참조하고자 하는 컬럼의 타입과 일치해야 함
...
-- RESTRICT : 참조되고 있는 테이블의 데이터가 삭제 또는 수정을 불가능하게 함
...
-- CHECK 제약조건이 걸린 컬럼에 조건에 해당하지 않는 값을 삽입, 수정할 수 없음
INSERT INTO check_table VALUES(1, '남');
INSERT INTO check_table VALUES(1, '남자');
