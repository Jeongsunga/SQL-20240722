...
-- 집계 함수 : 여러 행의 레코드를 종합하여 하나의 결과 값을 반환
...
SELECT SUM(amount) FROM sale; -- 컬럼을 무조건 선택
...
-- 그룹화한 컬럼과 집계함수를 제외한 컬럼은 SELECT절에서 사용이 불가능
SELECT AVG(amount), date
FROM sale
GROUP BY employee_number; -- 오류
...
-- 존재하지 않는 컬럼은 HAVING 절에서 사용할 수 없음
SELECT 
	AVG(amount) '평균', 
	employee_number '사번'
FROM sale
