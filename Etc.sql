...
-- CASE 컬럼 
-- WHEN 비교값1 THEN 비교값1과 컬럼이 일치할 때의 결과 
-- WHEN 비교값2 THEN 비교값2와 컬럼이 일치할 때의 결과
-- ELSE 비교값1과 비교값2가 모두 컬럼과 일치하지 않을 때의 결과
-- END
...
-- IF 함수 : 첫 번째 매개변수로 전달한 표현식에 따라 결과를 반환하는 함수
-- IF (조건, 참일 때 결과, 거짓일 때 결과)
SELECT date, IF(amount > 100000, '고액', '소액') '금액'
FROM sale;

-- IFNULL 함수 : 첫 번째 매개변수의 값이 NULL인지에 따라 결과를 반환하는 함수
-- IFNULL(값, 값이 null일 때 반환할 결과)
SELECT 사번, 사원이름, 부서명, IFNULL(부서명, '없음')
FROM employee_view;
